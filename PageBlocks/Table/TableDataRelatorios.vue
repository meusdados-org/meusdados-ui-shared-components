...
export default {
    ...
    watch: {
        ...
        entries: {
            handler(newRows) {
                this.tableEntries = newRows.map((row, index) => ({
                    ...row,
                    status: index === 0 ? true : false
                }));
                if (this.$refs.pagination?.updatePageCounting) {
                    this.$refs.pagination.updatePageCounting(this.tableEntries);
                }
            },
            immediate: true
    ...
    methods: {
        ...
        formatStatus(status) {
            /*
                Função que recebe o status de um relatório e retorna uma string, que aplica uma classe CSS.
            */
    ...
    created() {
        this.tableEntries = this.entries.map((row, index) => ({
            ...row,
            status: index === 0 ? true : false
        }));
    },
