...
import { PoliticaPrivacidadeService } from '@/services/politica-privacidade';

const itemsPerTable = 10;

export default {
    name: "TableDataPoliticasPrivacidade",
    props: {
        columns: Array,
        entries: Array,
        service: CallBackend,
    ...
    methods: {
        ...
        async changeAtivo(td) {
 
            td.ativo = !td.ativo;

            try {
                const polCount = await new PoliticaPrivacidadeService().getInfo();
                const polsActive = polCount.data.Politicas_Ativas;

            if (polsActive >= 1 && td.ativo === true) {
                this.$dialog({
                title: 'Limite atingido',
                message: 'O número máximo de políticas ativas já foi atingido.',
                type: 'alert'
            });
                td.ativo = !td.ativo;
                return;
            ...
            await new this.service().updateAtivo(td.id, td);
