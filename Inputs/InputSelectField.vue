  ...
  <div class="wrapper-InputSelectField">
    <div class="notFormGroupVersion" :class="{ formGroupVersion }">
      <multiselect
        ...
        selectedLabel=""
        :id="id"
        :openDirection="formGroupVersion ? 'bottom' : openDirection"
...
export default {
  ...
  props: {
    placeholder: String,
    value: String, // necessário para funcionar com v-model:value
    maxHeight: Number,
    allowEmpty: {
      type: Boolean,
      default: true
  ...
  data() {
    return {
      id: `inputSelectField-${this.placeholder?.toLowerCase().replace(/\s/g, '-') || 'select'}`,
      error: false
    };
  },
  methods: {
    ...
    onUpdateModelValue(val) {
      this.$emit('update:value', val); // repassa a mudança pro pai
    }
  }
};
</script>
...
.formGroupVersion .multiselect,
.formGroupVersion .multiselect--above {
  min-height: 18px;
  width: 100%;
}
